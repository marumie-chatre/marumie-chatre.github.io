...
export const metadata = {
  ...
  description: "新宿は約50店舗ある業界最大の激戦区。激戦区だからこそ気をつけたい選び方の軸と、主要7社の比較表、評価が高い事務所4社を、編集部が独自に整理した新宿ガイドです。",
};
  ...
  return (
    <main>
      ...
      <article className="col-article-body">
        ...
        <p>ここでは、編集部が各事務所の公式情報を集約して整理した <strong>「新宿の事務所マップ」</strong> と、選び方の軸、実際に新宿で働いた方の声まで、迷わず判断できるようにまとめていきますね。</p>

        <PointBox title="このページの情報源">
          ・各事務所の公式サイト（店舗情報・サービス内容）<br />
          ・編集部が運営する <Link href="/kuchikomi">口コミデータ</Link>（実際に新宿で働いた方の声）<br />
          ・業界共通の知見（報酬率・身バレ対策・面談時の見分け方など）<br />
          ※新宿エリアでの口コミも一部含まれていますが、客観的な情報集約を中心にまとめています
        </PointBox>
        ...
        <p style={{fontSize:"12px", color:"#888", marginTop:"8px"}}>★ = 編集部の新宿推奨4社。◆ = 新宿エリアならではの特色がある事務所（美容機器・全店女性スタッフ等）。「サポート密度」は、各社公式の研修・スタッフ体制・口コミでの相談しやすさ評価を編集部が3段階で集約したものです。</p>
        ...
        <h2>編集部の独自視点：新宿で「ハズレ事務所」を引かないために</h2>
        <p>新宿のように事務所が集中しているエリアほど、<MarkerSpan>「面談での即答度」で事務所の本質が見えます</MarkerSpan>。</p>
        ...
        <PointBox title="編集部が確認している『ノンアダ即答度』">
          ...
          口コミでも、ポケットワーク（id:44, 48）・アスタリスク（id:41, 43）でこのパターンの体験談が報告されています。一方、フェアリーテイル・アット・ブライト・チャトスタでは「ノンアダ希望が尊重された」という声が中心です。
        </PointBox>
        ...
        <TipBox title="実際に働いた方の声（id:12・★4）">
        ...
        <TipBox title="実際に新宿店で働いた方の声（id:36・★4）">
        ...
        <p>新宿駅徒歩5分という好立地で、<MarkerSpan>全店女性スタッフのみ</MarkerSpan>という事務所。スタッフが全員女性だと相談のハードルが下がる、というのが一番のメリットなんです。ただし、口コミでは「<strong>稼ぎ重視の熱量が高い</strong>」「<strong>アダルトの示唆を感じることがあった</strong>」（id:41, 43）という声もあるので、<strong>ゆるく副業として続けたい方には合わない可能性</strong>があります。本気で稼ぎたい方向きです。</p>
